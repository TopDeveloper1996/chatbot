<template>
    <div class="bg-[#232b46] p-4 rounded-md mb-4">
        <TeamsystemChartsVisataOptions :onOptionChange="(e) => changeAggregationMode(e)" 
            :title="`Focus costi variabili`"
            :mode="props.mode"
            :id="`focus_costi_variabili`" />
        <apexchart
...
const quartertlyData = convertToQuarterly(props.chartsData.costi_variabili.ytd_values);
const lastQuartertlyData = convertToQuarterly(props.chartsData.costi_variabili.lytd_values);
...
const ytdSeries = computed(() => {
    return aggrigationMode.value === "monthly" ? props.chartsData.costi_variabili.ytd_values : quartertlyData;
});

const lytdSeries = computed(() => {
    return aggrigationMode.value === "monthly" ? props.chartsData.costi_variabili.lytd_values : lastQuartertlyData;
});
...
const emit = defineEmits<{ (e: 'costi_variabili_chart-org-completed', result: any): void }>();

watch(() => props, (newValue) => {
    const bounds = getMaxMin([...props.chartsData.costi_variabili.ytd_values, ...props.chartsData.costi_variabili.lytd_values], 0.1);
    const result = {
        name: "Focus costi variabili",
        type: "line",
        ...
        "Anno corrente": props.chartsData.costi_variabili.ytd_values,
        "Anno precedente": props.chartsData.costi_variabili.lytd_values
    };
    emit('costi_variabili_chart-org-completed', result);
}, { immediate: true });
</script>
