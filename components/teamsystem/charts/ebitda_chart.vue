...
const quartertlyData = convertToQuarterly(props.chartsData.ebitda.ytd_values);
const lastQuartertlyData = convertToQuarterly(props.chartsData.ebitda.lytd_values);
...
const ytdSeries = computed(() => {
    return aggrigationMode.value === "monthly" ? props.chartsData.ebitda.ytd_values : quartertlyData;
});

const lytdSeries = computed(() => {
    return aggrigationMode.value === "monthly" ? props.chartsData.ebitda.lytd_values : lastQuartertlyData;
});
...
watch(() => props, (newValue) => {
    const bounds = getMaxMin([...props.chartsData.ebitda.ytd_values, ...props.chartsData.ebitda.lytd_values], 0.1);
    const result = {
        name: "Focus EBITDA",
        type: "line",
        ...
        "Anno corrente": props.chartsData.ebitda.ytd_values,
        "Anno precedente": props.chartsData.ebitda.lytd_values
    };
