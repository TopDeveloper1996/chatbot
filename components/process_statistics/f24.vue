<template>
    <div v-if="stats" class="w-full flex flex-col items-stretch justify-start gap-2">
        <h2 class="text-center">Statistiche F24</h2>
        ...
        <div class="flex flex-row w-full items-start gap-4">
            <div class="flex-initial flex pt-12 flex-col items-stretch gap-2 max-w-[30rem]">
                <div class="flex flex-row items-center gap-2">
                    <div class="card">
                        <p class="title">Numero F24</p>
            ...
            <PaginatedTable
                ...
                :sorted-headers="[
                            ...
                            {title:'Numero F24',key:(val:any)=>val.count},
    { title: 'Data ultimo F24', key: (val: any) => val.lastDoc.data_versamento },
    { title: 'Data ultimo download', key: (val: any) => val.lastDownload.created_at },
                ...
                :headers="['Numero ditta', 'Nome ditta', 'Numero F24', 'Data ultimo F24', 'Data ultimo download']"
                :page-size="10"
                :row-classes="(e: any) => {
                ...
                :row-mapper="(d: any) => [d.ditta_numero, d.ditta_registered_name ,d.count, d.lastDoc.data_versamento.toLocaleDateString(),d.lastDownload.created_at.toLocaleDateString()]"
            >
            </PaginatedTable>
            <PaginatedTable
                class="min-w-[16rem]"
        ...
        <h3 class="text-start">F24 ordinati per data documento</h3>
        <apexchart
        ...
        ></apexchart>
        <h3 class="text-start">F24 ordinati per data lettura file e stoccaggio dati in memoria</h3>
        <apexchart
            class="flex-1 max-w-[100%]"
            width="100%"
            :height="bp.greaterOrEqual('lg') ? '300px' : '150px'"
        ...
        ></apexchart>
        <h3 class="text-start">F24 ordinati per data scarico dal cassetto fiscale</h3>
        <apexchart
            class="flex-1 max-w-[100%]"
            width="100%"
            :height="bp.greaterOrEqual('lg') ? '300px' : '150px'"
...
import { F24ProcessStatistics } from "~/src/types/process_statistics_types";
...
const stats = ref<F24ProcessStatistics>();
...
function updateStats() {
    setTimeout(() => {
        stats.value = processStatistics.computeF24Statistics({
