<template>
    <div v-if="isUserProvideDataYTD || isUserProvideDataLYTD">
        ...
        <h4 class="text-xl font-bold">Utile di periodo</h4>
        <Tip>
            <p>
...
const timespant = computed(() => {
   ...
   return {
           min: l.min([...props.dittaStatsSeriesytd.profitto_netto, ...props.dittaStatsSerieslytd.profitto_netto, 0]),
           max: l.max([...props.dittaStatsSeriesytd.profitto_netto, ...props.dittaStatsSerieslytd.profitto_netto]),
       }
...
const data = computed(() => {
       return [
            {
                name: "Anno corrente",
                data: props.dittaStatsSeriesytd.profitto_netto,
                type: "line",
                color: "#76419F",
            },
            {
                name: "Anno precedente",
                data: props.dittaStatsSerieslytd.profitto_netto,
                type: "line",
                color: "#1AA1E0",
            },
        ];
...
const emit = defineEmits<{ (e: 'utile_di_periodo_chart-org-completed', result: any): void }>();

watch(() => props, (newValue) => {
        const bounds = getMaxMin([...props.dittaStatsSeriesytd.profitto_netto, ...props.dittaStatsSerieslytd.profitto_netto ], 0.1);
        const result = {
            ...
            anno_corrente: props.dittaStatsSeriesytd.profitto_netto,
            anno_precedente: props.dittaStatsSerieslytd.profitto_netto,
        };
        emit('utile_di_periodo_chart-org-completed', result);
}, { immediate: true });
