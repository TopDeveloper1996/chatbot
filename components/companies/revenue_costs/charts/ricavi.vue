<template>
    <div>
        <h4 class="text-xl font-bold">Focus ricavi</h4>
        <Tip>
            <p>
...
const timespant = computed(() => {
   ...
   return {
           min: l.min([...props.dittaStatsSeriesytd.ricavi, ...props.dittaStatsSerieslytd.ricavi, 0]),
           max: l.max([...props.dittaStatsSeriesytd.ricavi, ...props.dittaStatsSerieslytd.ricavi]),
       }
...
const data = computed(() => {
       return [
            {
                name: "Anno corrente",
                data: props.dittaStatsSeriesytd.ricavi,
                type: "line",
                color: "#76419F",
            },
            {
                name: "Anno precedente",
                data: props.dittaStatsSerieslytd.ricavi,
                type: "line",
                color: "#1AA1E0",
            },
        ];
...
const emit = defineEmits<{ (e: 'focus_ricavi_chart-org-completed', result: any): void }>();

watch(() => props, (newValue) => {
        const bounds = getMaxMin([...props.dittaStatsSeriesytd.ricavi, ...props.dittaStatsSerieslytd.ricavi ], 0.1);
        const result = {
            name: "Focus ricavi",
            type: "line",
            ...
            anno_corrente: props.dittaStatsSeriesytd.ricavi,
            anno_precedente: props.dittaStatsSerieslytd.ricavi,
        };
        emit('focus_ricavi_chart-org-completed', result);
}, { immediate: true });
